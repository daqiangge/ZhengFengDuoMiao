...
- (int)getHeightWithFramesetter:(CTFramesetterRef)framesetter width:(CGFloat)width
{
    ...
    NSLog(@"***%f",suggestedSize.width);
    
    return suggestedSize.height+1;
}

- (CGSize)getSizeWithFramesetter:(CTFramesetterRef)framesetter maxSize:(CGSize)maxSize
{
    if (_attString == nil)
    {
    ...
    // 获得建议的size
    CGSize suggestedSize = CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(framesetter, _attString, maxSize, _numberOfLines);
